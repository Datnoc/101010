...
import { getAlpacaCryptoBars, isAlpacaConfigured } from '@/lib/alpaca';
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Cache key oluştur
    const cacheKey = `${symbol.toUpperCase()}-${timeframe}-${period}`;
    ...
    const bars = await getAlpacaCryptoBars(symbol.toUpperCase(), {
    ...
    console.error('Alpaca crypto market data error:', error);
    return NextResponse.json(
      { 
